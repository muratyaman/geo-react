...
export const haversineDistanceInKM = (pos1, pos2) => {
  ...
  const [rlat1, rlat2, rlon1, rlon2] = [lat1, lat2, lon1, lon2].map(x => deg2rad(x)),
        dLat = rlat2 - rlat1,
        dLon = rlon2 - rlon1,
        radius = 6372.8; // Earth's radius in km

  return round(radius * 2.0 * asin(sqrt(pow(sin(dLat / 2.0), 2.0) + pow(sin(dLon / 2.0), 2.0) * cos(rlat1) * cos(rlat2))) * 1000.0) / 1000.0;
};
/**
